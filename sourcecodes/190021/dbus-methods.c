 ...
 *
 * To compile: $ gcc -Wall -g dbus-methods.c -o dbus-methods `pkg-config --libs --cflags dbus-1`
 * Usage: $ ./dbus-methods
 *
...
void dbus_server() {
    ...
    bool loop = TRUE;
    // Loop when something an be read or writen
    while (dbus_connection_read_write_dispatch(conn, -1)) {
        ...
        if (!dbus_message_has_path(message, OBJECT_PATH_NAME)) {
            printf("Server: Wrong object path.\n");
            dbus_message_unref(message);
            continue;
        }
        ...
        if (dbus_message_is_method_call(message, INTERFACE_NAME, METHOD_NAME_YES)) {
            sprintf(answer, "Yes, %s", s);
        } else if (dbus_message_is_method_call(message, INTERFACE_NAME, METHOD_NAME_NO)) {
            sprintf(answer, "No, %s", s);
        } else if (dbus_message_is_method_call(message, INTERFACE_NAME, METHOD_NAME_QUIT)) {
